...
@PyroX.on_message(filters.command(["promote","fpromote"], prefixes=config.HANDLER) & filters.user(config.OWNER_ID))
...
@PyroX.on_message(filters.command(["pin","unpin"], prefixes=config.HANDLER) & filters.user(config.OWNER_ID))
...
@PyroX.on_message(filters.command("invite", prefixes=config.HANDLER) & filters.user(config.OWNER_ID))
async def invite_link(_, message):
     chat_id = message.chat.id
     try:
        link = (await PyroX.get_chat(chat_id)).invite_link
     except Exception as e: 
         return await message.edit(f"Somthing Wrong Happens:\n{e}")
     return await message.edit(str(link))

@PyroX.on_message(filters.command("admins", prefixes=config.HANDLER) & filters.user(config.OWNER_ID))
async def admins_list(_, message):
     ...
     try:
        async for m in PyroX.get_chat_members(chat_id, filter=enums.ChatMembersFilter.ADMINISTRATORS):
...
@PyroX.on_message(filters.command("del", prefixes=config.HANDLER) & filters.user(config.OWNER_ID))
...
@PyroX.on_message(filters.command("ban", prefixes=config.HANDLER) & filters.user(config.OWNER_ID))
async def ban_member(_, message):
    ...
    try:
       owo = await message.chat.ban_member(user_id)
        ...
        return await message.edit(f"Somthing wrong Happens:\n{e}")
    name = (await PyroX.get_users(user_id)).first_name
    return await message.edit(f"=> {name} Has Been Banned!")


@PyroX.on_message(filters.command("unban", prefixes=config.HANDLER) & filters.user(config.OWNER_ID))
...
@PyroX.on_message(filters.command("purge", prefixes=config.HANDLER) & filters.user(config.OWNER_ID))
async def purge(_, message):
    ...
    if not message.reply_to_message:
        ...
        try:
           await PyroX.delete_messages(chat_id=chat_id, message_ids=message_ids)
        except Exception as e:
              return await message.edit(f"Somthing wrong Happens:\n{e}")
