...
@PyroX.on_message(filters.user(OWNER_ID) & filters.command("logs",prefixes=HANDLER))
async def logs(_, message):
       run_logs = run("tail logs.txt")
       msg = await message.edit_text("Analyzing Logging...")
       thumb_id = "./PyroX/PyroX_img/IMG_20230503_093609_915.jpg"
...
@PyroX.on_message(filters.user(OWNER_ID) & filters.command("sh",prefixes=HANDLER))
...
@PyroX.on_message(filters.user(OWNER_ID) & filters.command("e",prefixes=HANDLER))
