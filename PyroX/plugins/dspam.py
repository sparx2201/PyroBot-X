...
@PyroX.on_message(filters.command(["ds"], prefixes=config.HANDLER) & filters.user(config.OWNER_ID))
async def delay_handler(_, m: Message):
    try:
        ...
        if len(m.command) < 3:
            await PyroX.send_message(m.chat.id, f"Use like this: `{config.HANDLER}dspam [count spam] [delay time in seconds] [text messages]`")
...
@PyroX.on_message(filters.command(["spam"], prefixes=config.HANDLER) & filters.user(config.OWNER_ID))
async def spam_handler(_, m: Message):
    try:
        ...
        if not reply and len(cmd) < 2:
            await PyroX.send_message(m.chat.id, f"Use like this: {config.HANDLER}spam [count spam] [text messages]")
            return
